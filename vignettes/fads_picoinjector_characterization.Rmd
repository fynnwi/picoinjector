...
d <- dropletSummary %>% 
  ...
  mutate("water_oil_ratio" = p_water/(q_emulsion+q_oil_est),
         "injected_volume" = sapply(median_diameter * 1e-6, get_volume_change, 30e-6) * 1e15)
m <- lm(injected_volume ~ water_oil_ratio, data = d)
summary(m)
cor(d$injected_volume, d$water_oil_ratio, method = "pearson")

d %>% 
  group_by(q_emulsion) %>% 
  summarise("dv_min" = min(injected_volume),
            "dv_max" = max(injected_volume))
```
